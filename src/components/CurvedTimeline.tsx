...
function buildCurvePath(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const peakX = centerX + sign * amplitude;

    const yStart = 40 + i * segmentHeight;
    const yEnd = 40 + (i + 1) * segmentHeight;
    const yMid = (yStart + yEnd) / 2;

    // Control points for a smooth S-curve segment
    const cp1x = centerX + sign * amplitude * 1.2;
    const cp1y = yStart + segmentHeight * 0.2;
    const cp2x = centerX + sign * amplitude * 1.2;
    const cp2y = yMid;

    d += ` C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${centerX} ${yEnd}`;

    // The milestone sits at the outermost point of the curve
    points.push({ x: peakX, y: yMid });
  }
